...
def parse_string_to_num(input_str:str):
    pattern = re.compile(r'(zero|one|two|three|four|five|six|seven|eight|nine)', re.IGNORECASE)
    
    def replace_with_num(match):
        return str(w2n.word_to_num(match.group(0)))
    output_string = pattern.sub(replace_with_num,input_str)
    #print(input_str," ",output_string)
    return output_string

def solve_day1(input:list):
    """
    ...
    for str in input:
        first_num = ''
        last_num = ''
        str_test = parse_string_to_num(str)
        ...
        if last_num == '':
            last_num = first_num
        print(first_num ,last_num ,str_test)
        result += int(first_num + last_num)
        #print(first_num,last_num,result)
    return result
